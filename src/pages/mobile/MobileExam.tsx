...
export default function MobileExam() {
  ...
  const [shuffleOptions, setShuffleOptions] = useState(() => {
    const saved = localStorage.getItem('tamrediano_shuffle_options');
    return saved !== null ? saved === 'true' : true;
  });

  useEffect(() => {
    localStorage.setItem('tamrediano_shuffle_options', String(shuffleOptions));
  }, [shuffleOptions]);
  ...
  const loadQuestionsForBank = async (bankId: string) => {
    ...
    try {
        ...
        const preparedQuestions = fetchedQs.map((q: any) => {
          let choices = q.options.map((opt: string, i: number) => ({ text: opt, originalIndex: i }));
          if (shuffleOptions) {
                    ...
                    {showModeSelect && (
                      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
                        ...
                        >
                          ...
                          <p className="text-gray-500 text-sm mb-6 font-medium">اختر الطريقة التي تفضلها في عرض الإجابات والشرح</p>
                          
                          {/* Toggle Shuffling Options */}
                          <div className="flex items-center justify-between bg-amber-50/50 p-4 rounded-2xl mb-6 border border-amber-100/50 text-right">
                            <div className="flex flex-col text-right pr-1">
                              <span className="font-bold text-gray-800 text-sm block">لغبطة ترتيب الاختيارات</span>
                              <span className="text-gray-500 text-[11px] font-semibold mt-0.5">تغيير عشوائي لترتيب الإجابات (A, B, C, D)</span>
                            </div>
                            <button 
                              type="button"
                              onClick={() => setShuffleOptions(!shuffleOptions)}
                              className={`relative w-12 h-6 rounded-full transition-colors duration-300 cursor-pointer flex-shrink-0 ${shuffleOptions ? 'bg-[#D4AF37]' : 'bg-gray-300'}`}
                            >
                              <span className={`absolute top-1 bottom-1 w-4 h-4 rounded-full bg-white shadow-sm transition-all duration-300 ${shuffleOptions ? 'left-7' : 'left-1'}`} />
                            </button>
                          </div>
